"""
Unit tests for AmazonBrScraper availability-first behavior.
"""
import pytest
...
class TestAmazonBrAvailabilityFirst:
    """Tests for availability check before price extraction."""
    
    @pytest.fixture
    def scraper(self):
        return AmazonBrScraper()
    
    @patch.object(AmazonBrScraper, 'fetch_html')
    def test_unavailable_product_skips_price_extraction(self, mock_fetch, scraper):
        ...
        result = scraper.scrape_price(
            url='https://www.amazon.com.br/test',
    ...
    @patch.object(AmazonBrScraper, 'fetch_html')
    def test_amazon_specific_unavailable_messages(self, mock_fetch, scraper):
        """Test Amazon-specific unavailability messages."""
        unavailable_phrases = [
